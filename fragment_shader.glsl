...
TraceResult traceOneProbeOct(in LightFieldSurface L, in ProbeIndex index, in Ray worldSpaceRay, inout float tMin, inout float tMax, inout vec2 hitProbeTexCoord) {
    ...
    if (all(equal(probeSpaceRay.origin, vec3(0.0)))) {
        ...
        Vector3 probeSpaceStartPoint = probeSpaceRay.direction * rayBumpEpsilon;
        Vector3 probeSpaceEndPoint = probeSpaceRay.direction * (tMax - rayBumpEpsilon);

        hitProbeTexCoord = (octEncode(normalize(probeSpaceStartPoint)));
        return TRACE_RESULT_HIT;
    } else {
        // The origin of probeSpaceRay is not (0, 0, 0)
        // Do something else...
...
bool traceProject(LightFieldSurface L, Ray worldSpaceRay, inout float tMax, out Point2 hitProbeTexCoord, out ProbeIndex hitProbeIndex) {
    hitProbeIndex = -1;
    ...
    while (probesLeft > 0) {
        TraceResult result = traceOneProbeOct(L, i, worldSpaceRay, tMin, tMax, hitProbeTexCoord);
...
void main() {
    ...
    // Example: Sample the radianceProbeGrid texture
    // vec4 radianceColor = texture(L1.radianceProbeGrid, vec3(uv, 0.0)); // Using uv as texture coordinates, assuming it's in the range [0, 1]

    // Combine the ray direction and radiance color
    // map the components of the normalized ray direction from[-1, 1] to the range [0, 1]. 
